...
class Branch {
	...
	public function getByOstan() {$b = 'SELECT code,name FROM tbl_branch WHERE ostan="' . $_SESSION['ostan'] . '"';
		$c = new CDatabase;
	...
	public function getNameById($d) {
		$c = new CDatabase;
		$d = $c -> escape($d);
		$b = 'SELECT name FROM tbl_branch WHERE code="' . $d . '" AND ostan="' . $_SESSION['ostan'] . '"';
		$e = $c -> queryOne($b);
		if ($e)
			return $e -> name;
		return FALSE;
